...
module RegexDojo
  module Components
    module Screens
      ...
      # into Tailwind, using the shared RegexDojo::Components::Ui:: components.
          ...
          div(class: "flex items-center justify-between h-[66px] px-6 bg-white border-b border-dojo-violet-border") do
            div(class: "flex items-center gap-7") do
              ...
              nav(class: "flex gap-1") do
                button(class: "tab-btn text-sm font-semibold px-3.5 py-2 rounded-xl text-dojo-slate hover:bg-dojo-violet-light hover:text-dojo-violet cursor-pointer", data: {action: "click->tabs#switch", tabs_target: "tab", tab: "home"}) { "Dojo" }
                button(class: "tab-btn text-sm font-semibold px-3.5 py-2 rounded-xl text-dojo-slate hover:bg-dojo-violet-light hover:text-dojo-violet cursor-pointer", data: {action: "click->tabs#switch", tabs_target: "tab", tab: "playground"}) { "Playground" }
                button(class: "tab-btn text-sm font-semibold px-3.5 py-2 rounded-xl text-dojo-slate hover:bg-dojo-violet-light hover:text-dojo-violet cursor-pointer", data: {action: "click->tabs#switch", tabs_target: "tab", tab: "challenges"}) { "Challenges" }
                button(class: "tab-btn text-sm font-semibold px-3.5 py-2 rounded-xl text-dojo-slate hover:bg-dojo-violet-light hover:text-dojo-violet cursor-pointer", data: {action: "click->tabs#switch", tabs_target: "tab", tab: "leaderboard"}) { "Leaderboard" }
              end
            end
            div(class: "flex items-center gap-3.5") do
              span(class: "flex items-center gap-1.5 bg-orange-50 text-orange-700 font-bold text-sm px-3 py-1.5 rounded-xl border border-orange-200") { "🔥 #{@user.streak} day streak" }
              button(
                class: "w-9 h-9 rounded-full bg-gradient-to-br from-pink-400 to-pink-600 text-white font-bold text-sm flex items-center justify-center cursor-pointer border-0",
                data: {action: "click->tabs#switch", tabs_target: "tab", tab: "profile"}
              ) do
                # Deriving initials from session_id
        ...
        def hero_row
          div(class: "flex gap-5 mb-5") do
            ...
            div(class: "flex-[1.6] rounded-[22px] p-7 bg-gradient-to-br from-dojo-violet to-dojo-violet-dark text-white relative overflow-hidden") do
              render RegexDojo::Components::Ui::Pill.new(bg: "bg-white/20", text: "text-white") { "☀️ DAILY KATA" }
              ...
              render RegexDojo::Components::Ui::Button.new(variant: "on-white", data: {action: "click->tabs#switch", tab: "lesson"}) { "Start today's kata →" }
        ...
        def continue_panel
          div(class: "bg-white border border-dojo-violet-border rounded-[20px] p-6") do
            ...
            div(class: "flex gap-3.5") do
              # 1
              div(class: "flex-1 border border-dojo-violet-border rounded-2xl p-[18px] bg-dojo-violet-wash") do
                render RegexDojo::Components::Ui::Pill.new(bg: "bg-dojo-success-bg", text: "text-dojo-success-text", class: "mb-3") { "✓ 3/5 DONE" }
                div(class: "font-bold text-[14.5px] mb-1") { "Character Classes" }
                div(class: "text-xs text-dojo-slate mb-3") { "Ranges, negation, shorthands" }
                div(class: "h-[7px] bg-dojo-violet-border rounded overflow-hidden") do
                  div(class: "w-[60%] h-full bg-dojo-success")
                end
              end
              # 2
              div(class: "flex-1 border border-dojo-violet-border rounded-2xl p-[18px]") do
                render RegexDojo::Components::Ui::Pill.new(bg: "bg-dojo-warning-bg", text: "text-dojo-warning-text", class: "mb-3") { "▶ IN PROGRESS" }
                div(class: "font-bold text-[14.5px] mb-1") { "Quantifiers & Greed" }
                div(class: "text-xs text-dojo-slate mb-3") { "*, +, ?, {n,m}, lazy vs greedy" }
                div(class: "h-[7px] bg-dojo-violet-border rounded overflow-hidden") do
                  div(class: "w-[30%] h-full bg-dojo-warning")
                end
              end
              # 3
              div(class: "flex-1 border border-dojo-violet-border rounded-2xl p-[18px] opacity-65") do
                render RegexDojo::Components::Ui::Pill.new(bg: "bg-dojo-violet-border", text: "text-dojo-violet", class: "mb-3") { "🔒 LOCKED" }
                div(class: "font-bold text-[14.5px] mb-1") { "Groups & Backrefs" }
                div(class: "text-xs text-dojo-slate mb-3") { "Capturing, named, \\1 refs" }
                div(class: "h-[7px] bg-dojo-violet-border rounded overflow-hidden") do
                  div(class: "w-0 h-full bg-dojo-violet")
                end
              end
            end
          end
