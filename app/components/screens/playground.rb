  ...
  module Components
    module Screens
      ...
      class Playground < Phlex::HTML
        ...
        def view_template
          render RegexDojo::Components::Ui::Card.new(data: {controller: "regex-playground"}) do
            topbar
        ...
        def topbar
          div(class: "flex items-center justify-between h-[66px] px-6 bg-white border-b border-dojo-violet-border") do
            ...
            div(class: "flex gap-2") do
              render RegexDojo::Components::Ui::Button.new(variant: :ghost, data: {action: "regex-playground#reset"}) { "↺ Reset" }
              render RegexDojo::Components::Ui::Button.new { "Run tests" }
            end
          end
        end
