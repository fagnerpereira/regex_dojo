...
export default class extends Controller {
  ...
  renderPreview(testString, matches, error) {
    ...
    for (const m of matches) {
      if (m.start > cursor) {
        this.previewTarget.append(document.createTextNode(testString.slice(cursor, m.start)));
      }
      const mark = document.createElement("mark");
      mark.className = "bg-amber-200 text-amber-900 rounded px-0.5 font-semibold";
      mark.textContent = m.text;
      this.previewTarget.append(mark);
      cursor = m.end;
    }
    if (cursor < testString.length) {
      this.previewTarget.append(document.createTextNode(testString.slice(cursor)));
    }
  }
  ...
  renderMatchList(matches, error) {
    ...
    this.successBannerTarget.classList.toggle("hidden", error || matches.length === 0);
